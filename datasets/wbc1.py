...
def get_wbc1_ood_test_dataset(id_label=1):
    df = pd.read_csv('wbc1_test_dataset.csv')
    df = df[df['class label'] != 5]
    ...
    test_data.to_csv('wbc1_ood_test_dataset.csv', index=False)

    test_dataset = WBC_dataset1(csv_path='wbc1_ood_test_dataset.csv', images_path='wbc/segmentation_WBC/Dataset 1')

    count_unique_labels_of_dataset(test_dataset, "wbc1_ood_test_dataset")
