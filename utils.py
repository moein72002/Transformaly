...
from datasets.rsna import get_rsna_trainset, get_rsna_test_set, get_rsna_test_set_id, get_rsna_test_set_ood, get_rsna_just_test_shifted
from datasets.isic import get_isic_test_set, get_isic_test_set_ood, get_isic_trainset, get_isic_test_set_id, get_isic_just_test_shifted
...
def extract_fetures(base_path,
    ...
    for dataset in datasets:
        ...
        if manual_class_num_range is not None:
            ...
            if dataset in ['mvtec']:
                _classes = [0]
            elif dataset in ['br35h', 'brats2015', 'camelyon17', 'rsna', 'isic']:
                _classes = [0]
        ...
        for _class in _classes:
            ...
            for unimodal in unimodal_vals:
                ...
                assert dataset in ['camelyon17', 'rsna', 'isic', 'mvtec', 'br35h', 'brats2015', 'wbc1', 'wbc2', 'cifar10', 'cifar100', 'fmnist', 'cats_vs_dogs',
                ...
                if dataset == 'mvtec':
                    ...
                    trainset = get_rsna_trainset()
                    testset = get_rsna_test_set()
                    anomaly_targets = testset.labels
                    just_testset = get_rsna_just_test_shifted()
                    just_test_anomaly_targets = just_testset.labels
                elif dataset == 'isic':
                    trainset = get_isic_trainset()
                    testset = get_isic_test_set()
                    anomaly_targets = testset.labels
                    just_testset = get_isic_just_test_shifted()
                    just_test_anomaly_targets = just_testset.labels
                elif dataset == 'wbc1':
                ...
                if dataset in ['wbc1', 'wbc2', 'br35h', 'brats2015', 'camelyon17', 'rsna', 'isic']:
...
def train(model, best_model, args, dataloaders,
    ...
    for epoch in tqdm(range(1, args['epochs'] + 1)):
        ...
        if args['test_every_epoch'] and (epoch - 1) % args['eval_every'] == 0:
            ...
            if args['dataset'] in ['wbc1', 'wbc2', 'br35h', 'brats2015', 'mvtec', 'camelyon17', 'rsna', 'isic']:
                manual_class_num_range = None
            else:
...
def evaluate_method(args=None, BASE_PATH=None, _classes=None, mvtec_category=None):
    ...
    for _class in _classes:
        ...
        if args['dataset'] in ['wbc1', 'wbc2', 'br35h', 'brats2015', 'camelyon17', 'rsna', 'isic']:
        ...
        if args['dataset'] in ['wbc1', 'wbc2', 'br35h', 'camelyon17', 'brats2015', 'rsna', 'isic']:
        ...
        if args['dataset'] in ['wbc1', 'wbc2', 'br35h', 'camelyon17', 'rsna', 'isic', 'brats2015']:
